...
const SearchLogic1 = async (req, res) => {
  try {
    ...
    return Fail(res, 500, "예약 목록 조회 중 오류가 발생했습니다.");
  }
};
...
const SearchLogic2 = async (req, res) => {
  try {
    ...
    if (error.statusCode === 404) {
      return Fail(res, 404, "예약을 찾을 수 없습니다.");
    }
    console.error("Get booking error:", error);
    return Fail(res, 500, "예약 조회 중 오류가 발생했습니다.");
  }
};
...
const SaveLogic1 = async (req, res) => {
  try {
    ...
    return Fail(res, 500, "예약 생성 중 오류가 발생했습니다.");
  }
};
...
const SaveLogic2 = async (req, res) => {
  try {
    ...
    if (error.statusCode === 400) {
      return Fail(res, 400, "이미 취소된 예약입니다.");
    }
    console.error("Cancel booking error:", error);
    return Fail(res, 500, "예약 취소 중 오류가 발생했습니다.");
  }
};
